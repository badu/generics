...
func TestMapWhere(t *testing.T) {
	...
	rt3 := Map(t3, func(d *Data, i int) string {
		return d.Title
	})
...
func TestProduce(t *testing.T) {
	...
	t2 := Produce(3, func(i int) *Data {
		return &Data{Title: fmt.Sprintf("Title %d", i)}
	})
	is.Equal(len(t2), 3)
	is.Equal(t2, []*Data{{Title: "Title 0"}, {Title: "Title 1"}, {Title: "Title 2"}})
}
...
func TestUnique(t *testing.T) {
	...
	is.Equal(len(t2), 1)
	is.Equal(t2, []*Data{{ID: 1, OwnerID: 1, Title: "Title", Body: "Body", Quantity: 10}})
}
